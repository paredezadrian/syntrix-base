...
class DiagonalSSM(nn.Module):
    ...
    def __init__(self, d_model: int, state_size: int = 64, conv_kernel: int = 3, mlp_ratio: int = 4):
        super().__init__()
        self.norm = RMSNorm(d_model)
        self.conv = nn.Conv1d(d_model, d_model, kernel_size=conv_kernel, groups=d_model, padding=conv_kernel // 2)
