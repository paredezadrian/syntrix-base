...
def main(argv=None):
    ...
    p.add_argument("--config", type=str, default=None, help="Base YAML config (optional)")
    p.add_argument("--data.file", dest="data_file", type=str, required=True, help="Path to input text file")
    p.add_argument("--ckpt", type=str, required=False, help="Checkpoint path (if omitted, a fresh model is evaluated)")
    ...
    p.add_argument("--model", type=str, default="gpt_mini", help="Model type if building fresh model")
    p.add_argument("--block_size", type=int, default=128, help="Context length for evaluation")
    ...
    bpc = tr._Trainer__class__.__dict__["evaluate_bpc"](tr.model, tr.val_tokens, tr.args.block_size, iters=20, batch_size=16)  # type: ignore
